...
public class MemberController {						// 실제로는 JSON으로 보내준다. -> produces
	...
	@RequestMapping(value="member", method=RequestMethod.PUT, produces="application/json; charset=utf-8")// 반환시켜주는 데이터타입:produces)
	@ResponseBody
	...
	@RequestMapping(value="member", method=RequestMethod.DELETE, produces="application/json; charset=utf-8")// 반환시켜주는 데이터타입:produces)
	@ResponseBody
	public Map<String, Object> memberDelete(@PathVariable("no")int no, Model model){
		model.addAttribute("no", no);
