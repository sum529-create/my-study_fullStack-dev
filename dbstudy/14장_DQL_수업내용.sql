...
SELECT DEPARTMENT_NAME FROM DEPARTMENTS GROUP BY LOCATION_ID, DEPARTMENT_NAME;

-- << employees 테이블 >>

-- 34. 각 부서(department_id)별로 그룹화하여 department_id 와 부서별 사원의 수를 출력한다.
SELECT DEPARTMENT_ID, COUNT(DEPARTMENT_ID) FROM EMPLOYEES WHERE DEPARTMENT_ID IS NOT NULL GROUP BY DEPARTMENT_ID;
...
SELECT JOB_ID, COUNT(JOB_ID), AVG(SALARY) FROM EMPLOYEES GROUP BY JOB_ID, SALARY ORDER BY AVG(SALARY) ASC;

-- 37. 직업(job_id)이 SH_CLERK 인 직원들의 인원수와 최대급여 및 최소급여를 출력한다.
SELECT COUNT(JOB_ID), MAX(SALARY), MIN(SALARY) FROM EMPLOYEES WHERE JOB_ID = 'SH_CLERK' GROUP BY JOB_ID;

-- 38. 근무중인 사원수가 5명 이상인 부서의 department_id 와 해당 부서의 사원수를 department_id 의 오름차순으로 정렬하여 출력한다.
SELECT DEPARTMENT_ID, COUNT(DEPARTMENT_ID) FROM EMPLOYEES GROUP BY DEPARTMENT_ID HAVING COUNT(DEPARTMENT_ID) >= 5 ORDER BY DEPARTMENT_ID ASC;
...
SELECT DEPARTMENT_ID, JOB_ID, COUNT(JOB_ID) FROM EMPLOYEES GROUP BY DEPARTMENT_ID, JOB_ID ORDER BY DEPARTMENT_ID;
