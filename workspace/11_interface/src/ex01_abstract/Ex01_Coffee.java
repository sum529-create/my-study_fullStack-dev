 ...
 * 5. abstract class 클래스명{
 * 			abstract 결과타입 메소드명(매개변수);
 *    }
 * 6. 추상 클래스를 상속받는 서브 클래스들은
...
abstract class Coffee {
	...
	public abstract void taste();	// 추상메소드가 하나라도 있으면 그 클래스는 추상 클래스가 된다.
...
//		new Coffee("콜롭비아");		// 추상 클래스는 new를 할 수 없다.
		
		Coffee[] list = new Coffee[2]; 		// new아님 배열 잡을 때 사용하는 것[객체 생성하는 new가 아님]
